...
def diffraction_envelope_m(wavelength, alpha_prime, phi_prime, theta_o, theta_s):
    ...
    sinc_pit = np.sinc((w) * (np.sin(alpha) * np.cos(theta_s) + np.sin(phi) * np.cos(theta_o)) / wavelength)
    return (w)**2 * sinc_pit**2 + (Lambda - w)**2 * sinc_track**2 + 2 * (Lambda - w) * (w) * sinc_track * sinc_pit * np.cos(np.pi * (-Lambda / 2) * (np.sin(alpha) * np.cos(theta_s) + np.sin(phi) * np.cos(theta_o)) / wavelength + 2 * np.pi * n_pc(wavelength) * opl / wavelength)
